...
포크해서 쓰거나 사내에서 굴릴 때는 `config.json` 의 `updateUrl` 에 https 주소를 넣으면
...
- 브라우저로 여는 링크는 **네이버 · 업비트 · 한국은행 · GitHub 도메인만** 허용 (설정이 오염돼도 임의 URL 실행 불가)
...
이 프로젝트의 릴리스 파일은 **[SignPath Foundation](https://signpath.org/) 이 무료로 제공하는
코드 서명 인증서**로 서명한다. 서명은 [SignPath.io](https://about.signpath.io/) 의 서명 정책에 따라
**GitHub Actions 가 만든 산출물에만** 붙는다 — 서명이 보증하는 것은 '누가 만들었나' 가 아니라
'이 저장소의 이 소스로 만들었나' 이기 때문이다. 직접 빌드한 파일에 서명이 없는 것은 정상이다.

> Free code signing provided by [SignPath.io](https://about.signpath.io/),
> certificate by [SignPath Foundation](https://signpath.org/).

*(신청 진행 중. 승인 전 릴리스에는 서명이 없다.)*
...
[데이터 출처](#데이터-출처) 표에 적힌 곳이 전부다. 그중 두 가지만 짚어 둔다.
