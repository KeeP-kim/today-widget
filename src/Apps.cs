...
namespace DeskWidget
{
    ...
    internal static class Apps
    {
        ...
        /// </summary>
        /// <summary>COM 래퍼를 놓아준다. null 이거나 COM 이 아니면 아무 일도 하지 않는다.</summary>
        private static void Release(object com)
        {
            try { if (com != null && Marshal.IsComObject(com)) Marshal.ReleaseComObject(com); }
            catch (ArgumentException) { } catch (InvalidComObjectException) { }
        }
        ...
        public static List<InstalledApp> InstalledApps()
        {
            ...
            try
            {
                ...
                folder = t.InvokeMember("NameSpace", BindingFlags.InvokeMethod, null, shell,
                ...
                // ★ 최상위만 놓아주면 나머지는 GC 를 기다린다 ★
                //   여기서 받는 것은 전부 COM 래퍼다. 최대 600개를 훑는데 하나도 놓아주지
                //   않으면 shell32 쪽 네이티브 개체가 GC 가 돌 때까지 남는다. 이 앱은 대기
                //   상태로 오래 떠 있어 GC 가 거의 돌지 않는다.
        ...
        public static void Open(AppDef def)
        {
            ...
            try
            {
                ...
                // 받은 것을 놓아준다. 셸이 새 프로세스를 안 만들면 null 이 오는데 using 은 그것도 받는다
                // (Net.OpenLink 와 같은 꼴). 대기 상태에서는 GC 가 거의 안 돌아 핸들이 오래 남는다.
        ...
        private static ImageSource FromPngFile(string png)
        {
            try
            {
                ...
                // ★ DecodePixelWidth 는 상한이 아니라 '이 크기로 풀어라' 다 ★
                //   그냥 256을 주면 32픽셀짜리 그림도 256으로 키워서 푼다 - 메모리가 64배가 되고
                //   화면에는 억지로 늘린 그림이 나온다. 줄일 것이 있을 때만 준다.
                //   크기는 머리글만 읽어서 안다(OnDemand 라 화소를 풀지 않는다).
                int wide = 0;
                try {
                    using (var probe = File.OpenRead(png)) {
                        var dec = BitmapDecoder.Create(probe, BitmapCreateOptions.PreservePixelFormat, BitmapCacheOption.OnDemand);
                        if (dec.Frames.Count > 0) wide = dec.Frames[0].PixelWidth;
                    }
                } catch (NotSupportedException) { return null; } catch (ArgumentException) { return null; }
                ...
                // 갈아끼우는 쪽은 256으로 줄여 저장하지만, 이 파일은 사용자가 탐색기에서 같은
                // 이름으로 넣어도 그대로 읽힌다. 큰 것만 줄인다 - 가로만 주면 비율은 유지된다.
                if (wide > IconSide) bmp.DecodePixelWidth = IconSide;
        ...
        private static ImageSource FromIconLocation(string lnk)
        {
            ...
            try
            {
                ...
                o = t.InvokeMember("CreateShortcut", BindingFlags.InvokeMethod, null, shell,
                                   new object[] { lnk });
            ...
            finally
            {
                ...
                // 바로가기 개체도 놓아준다 - 전에는 최상위(shell)만 놓았다.
                Release(o); Release(shell);
            }
        }
    }
}
