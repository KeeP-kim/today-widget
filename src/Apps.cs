...
namespace DeskWidget
{
    ...
    internal static class Apps
    {
        ...
        /// <summary>
        /// 그림을 갈아끼운다. **256x256 PNG 만** 받는다.
        ///
        /// ★ 그리기 전에 머리글로 판정한다 ★
        ///   디코더에 먼저 넘겨 보고 크기를 재면, 남이 준 파일을 검사도 없이 해석하게 된다.
        ///   PNG 는 머리 24바이트에 서명과 크기가 그대로 들어 있으므로 그것만 읽어서 가린다.
        ///
        /// 원본을 가리키지 않고 보관소로 복사한다 - 원본이 지워져도 아이콘이 남아야 한다.
        /// </summary>
        public static bool SetIconOverride(string lnkPath, string png)
        {
            try
            {
                if (!IsAllowed(lnkPath) || string.IsNullOrEmpty(png)) return false;
                if (!File.Exists(png)) return false;
                if (!png.EndsWith(".png", StringComparison.OrdinalIgnoreCase)) return false;

                var fi = new FileInfo(png);
                if (fi.Length < 24 || fi.Length > 4 * 1024 * 1024) return false;   // 4MB 면 넉넉하다
                ...
                File.Copy(png, IconOverridePath(lnkPath), true);
                return true;
            }
            catch { return false; }
        }
