...
namespace DeskWidget
{
    ...
    internal sealed class Quote
    {
        ...
        /// ★ 환율은 휴일에도 '받아진다' ★ - 네이버가 전날 고시환율을 그대로 돌려주고 ReceivedUtc 만
        /// 새것이라, 거래일을 따로 두지 않으면 멈춘 값으로 예측을 채점하게 된다.
        /// </summary>
        public DateTime TradedDate;
        internal Quote Snapshot() { return (Quote)MemberwiseClone(); }
    }
    ...
    internal static class Sources
    {
        ...
        internal static string QuoteKey(SymbolDef def, string bank)
        { return def.Key + (def.Kind == SourceKind.Fx ? "|" + bank : ""); }
        internal static Quote SharedQuote(SymbolDef def, string bank)
        {
            lock (QuoteGate) { Quote quote; return SharedQuotes.TryGetValue(QuoteKey(def, bank), out quote) ? quote : null; }
        }
        internal static void PublishQuote(SymbolDef def, string bank, Quote quote)
        { PublishQuote(def, bank, quote, DateTime.UtcNow); }
        /// <summary>받은 시각을 밖에서 정해 넣는 형태. 검사가 늦게 도착한 옛 시세를 만들어 볼 수 있어야 한다.</summary>
        internal static void PublishQuote(SymbolDef def, string bank, Quote quote, DateTime stampUtc)
        {
            if (quote == null || !quote.Ok) return;
            quote.ReceivedUtc = stampUtc;
            string key = QuoteKey(def, bank);
            quote.IdentityKey = key;
            // ★ 시각을 잠금 밖에서 찍고 잠금 안에서 무조건 덮어썼다 ★
            //   위젯과 예측 창이 같은 품목을 동시에 부르면, 먼저 받은 값이 나중에 저장되어
            //   공유 시세가 뒤로 갈 수 있었다. 구독자는 시각으로 걸러 지켰지만, 사전을 읽는
            //   쪽(SharedQuote)은 옛 것을 받았다. 옛 것이면 저장도 통지도 하지 않는다.
            lock (QuoteGate)
            {
                Quote older;
                if (SharedQuotes.TryGetValue(key, out older) && older != null && older.ReceivedUtc > quote.ReceivedUtc) return;
                SharedQuotes[key] = quote;
            }
            var updated = QuoteUpdated;
            if (updated != null) updated(key, quote);
        }
        internal static async Task<Quote> FetchPredictionQuoteAsync(SymbolDef def, string bank, CancellationToken ct)
        {
        ...
        public static List<SymbolDef> Defaults()
        {
            return new List<SymbolDef>
            {
                ...
                new SymbolDef(SourceKind.Coin,       "KRW-BTC",   "비트코인"),
                new SymbolDef(SourceKind.WorldStock, "AAPL.O",    "애플"),
            };
        }
        ...
        {
            var quote = await FetchCoreAsync(def, bank, ct).ConfigureAwait(false);
            ct.ThrowIfCancellationRequested();
            PublishQuote(def, bank, quote);
            return quote;
        }

        private static async Task<Quote> FetchCoreAsync(SymbolDef def, string bank, CancellationToken ct)
        {
            switch (def.Kind)
            {
        ...
        private static async Task<Quote> FetchFx(SymbolDef def, string bank, CancellationToken ct)
        {
            ...
            return new Quote
            {
                ...
                TradedDate = DateOf(r["localTradedAt"].S),
        ...
        private static async Task<Quote> FetchWorld(SymbolDef def, CancellationToken ct)
        {
            ...
            {
                Ok = true,
                Price = WorldPrice(price, j["currencyType"]["code"].S),
                Unit = j["currencyType"]["code"].S ?? "현지통화",
        ...
        private static async Task<Quote> FetchUpbit(SymbolDef def, CancellationToken ct)
        {
            ...
            {
                Ok = true,
                // ★ 1원 미만 코인은 소수 첫째 자리에서 잘리면 전부 '0.0원' 이다 ★
        ...
        internal static string WorldPrice(string price, string currency)
        { return currency == "USD" ? "$" + price : price + " " + (currency ?? "현지통화"); }
        ...
        public static async Task<WeatherInfo> FetchWeatherAsync(double lat, double lon, CancellationToken ct)
        {
            ...
            // ★ 없는 값을 0 으로 바꾸면 '맑음 · 습도 0%' 를 지어낸다 ★
            //   Round0 은 NaN 을 0 으로 만든다. WMO 코드 0 은 '맑음' 이고 습도 0 은 그대로 찍힌다.
            //   날씨 코드를 모르면 이 관측 자체를 쓰지 않고, 습도는 -1(모름)로 두어 화면이 뺀다.
            double code = c["weather_code"].D;
            if (double.IsNaN(code)) return new WeatherInfo();
            double humidity = c["relative_humidity_2m"].D;
            ...
            return new WeatherInfo
            {
                ...
                Hum = double.IsNaN(humidity) ? -1 : (int)Math.Round(humidity),
        ...
        /// <summary>코인 가격에 필요한 소수 자릿수. 값이 작을수록 더 판다.</summary>
        internal static int CoinDigits(double price)
        {
            double p = Math.Abs(price);
            return p >= 1000 ? 0 : p >= 1 ? 1 : p >= 0.01 ? 4 : 6;
        }
        ...
        /// <summary>거래 시각 문자열에서 한국 날짜만. 못 읽으면 MinValue.</summary>
        internal static DateTime DateOf(string iso)
        {
            if (string.IsNullOrEmpty(iso)) return DateTime.MinValue;
            DateTimeOffset dt;
            if (!DateTimeOffset.TryParse(iso, CultureInfo.InvariantCulture, DateTimeStyles.AssumeLocal, out dt)) return DateTime.MinValue;
            return dt.ToUniversalTime().UtcDateTime.AddHours(9).Date;
        }
