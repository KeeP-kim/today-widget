...
namespace DeskWidget
{
    ...
    internal sealed class JNode
    {
        ...
        public bool IsObject { get { return _v is Dictionary<string, JNode>; } }
    ...
    internal static class Json
    {
        ...
        public static JNode Parse(string text)
        {
            ...
            try
            {
                ...
                SkipWs(text, ref i);
                if (i != text.Length) throw new FormatException("Trailing JSON content");
                return n;
            }
            catch
            {
        ...
        private static JNode ParseValue(string s, ref int i, int depth)
        {
            ...
            SkipWs(s, ref i);
            if (i >= s.Length) throw new FormatException("Missing JSON value");
        ...
        private static JNode ParseObject(string s, ref int i, int depth)
        {
            ...
            {
                SkipWs(s, ref i);
                if (i >= s.Length || s[i] != '"') throw new FormatException("Missing object key");
                string key = ParseString(s, ref i);
                SkipWs(s, ref i);
                if (i >= s.Length || s[i] != ':') throw new FormatException("Missing colon");
                i++;
        ...
        private static string ParseString(string s, ref int i)
        {
            ...
            {
                char c = s[i];
                if (c < ' ') throw new FormatException("Unescaped control character");
            ...
            while (i < s.Length)
            {
                ...
                {
                    i++;
                    if (i >= s.Length) throw new FormatException("Incomplete escape");
                    char e = s[i++];
                    switch (e)
                    {
                        ...
                        case 'u':
                            if (i + 4 > s.Length) throw new FormatException("Incomplete unicode escape");
                            for (int h = 0; h < 4; h++)
                            {
                                char hex = s[i + h];
        ...
        private static double ParseNumber(string s, ref int i)
        {
            ...
            if (i < s.Length && s[i] == '.')
            {
                ...
                int digits = i;
                while (i < s.Length && s[i] >= '0' && s[i] <= '9') i++;
                if (i == digits) throw new FormatException("Missing fraction digits");
            }
            if (i < s.Length && (s[i] == 'e' || s[i] == 'E'))
            {
                ...
                int digits = i;
                while (i < s.Length && s[i] >= '0' && s[i] <= '9') i++;
                if (i == digits) throw new FormatException("Missing exponent digits");
            }
            double d;
            if (double.TryParse(s.Substring(start, i - start), NumberStyles.Float,
                                CultureInfo.InvariantCulture, out d) && !double.IsInfinity(d)) return d;
