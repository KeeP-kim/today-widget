...
namespace DeskWidget
{
    ...
    internal static class Dock
    {
        ...
        public static List<ScreenInfo> AllScreens()
        {
            ...
            if (list.Count == 0)
            {
                ...
                // ★ 이 자리의 단위는 픽셀이다 ★
                //   Bounds/Work 는 '이 프로세스가 보는 픽셀' 로 정의돼 있고, 붙이기·자르기는
                //   창 DIP 에 배율을 곱해 이 값과 견준다. SystemParameters 는 DIP 라, 배율 200% 인
                //   화면에서 그대로 넣으면 화면이 실제의 절반으로 보여 창이 한가운데로 잘렸다.
                double dpi = SystemDpiScale();
                var whole = new Rect(0, 0, SystemParameters.PrimaryScreenWidth * dpi, SystemParameters.PrimaryScreenHeight * dpi);
                list.Add(new ScreenInfo { Bounds = whole, Work = whole, Phys = whole, Primary = true });
            }
            return list;
        }
        ...
        public static DockEdge DetectEdge(Visual v, Rect winDip, out ScreenInfo screen)
        {
            ...
            try
            {
                ...
                double snapWin = 72 * Math.Max(sx, sy);
                ...
                {
                    // ★ 화면 밖으로 넘어간 것은 '가장 확실한' 붙이기다 ★
                    //   전에는 절댓값으로만 재서, 창을 바 위에 푹 얹으면 오히려 거리가 멀어져
                    //   탈락했다. 그래서 커서를 화면 끝까지 내려야만 붙었다 - 큰 창은 머리를
                    //   바에 대야 해서 몹시 까다로웠다.
                    //   넘어간 쪽이 있으면 가장 많이 넘어간 변으로 붙인다.
                    double over, mostOver = 0;

                    over = scr.Bounds.Left - wl;   if (over > mostOver) { mostOver = over; edge = DockEdge.Left; }
                    over = scr.Bounds.Top - wt;    if (over > mostOver) { mostOver = over; edge = DockEdge.Top; }
                    over = wr - scr.Bounds.Right;  if (over > mostOver) { mostOver = over; edge = DockEdge.Right; }
                    over = wb - scr.Bounds.Bottom; if (over > mostOver) { mostOver = over; edge = DockEdge.Bottom; }

                    // 넘어간 데가 없으면 가까이 놓인 것만으로도 받아준다
                    if (edge == DockEdge.None)
                    {
                        double e;
    ...
    internal interface IDockBar
    {
        ...
        // BarCentered 는 지웠다. 가운데로 모으는 것은 '자리 나누기' 가 아니라 '내용' 의 일이다.
    ...
    internal static class DockStack
    {
        ...
        /// <summary>
        /// 바마다 '그 줄의 속이 찬 두께'. 창 두께가 아니다.
        ///
        /// 한 줄에 선 바들은 저마다 두께가 달라도 화면에서 뺏은 띠는 하나뿐이다.
        /// 그 띠의 깊이가 이 값이고, 바는 정확히 이만큼을 불투명하게 칠해야 한 줄로 이어진다.
        /// 더 두꺼운 몫(호버로 커질 자리)은 남의 자리 위라 그대로 비워 둔다.
        /// </summary>
        private static readonly Dictionary<IDockBar, double> _solid = new Dictionary<IDockBar, double>();
        ...
        private static void Layout(string device, DockEdge edge, bool remeasure)
        {
            ...
            // ★ 두께는 두 값뿐이다 ★
            //   t = 줄의 창 두께 (가장 두꺼운 것에 맞춘다. 호버로 커질 자리까지 품는다)
            //   s = 줄의 속이 찬 두께 (화면에서 뺏는 깊이이자, 바들이 칠해야 할 깊이)
            //   확보도 s, Fit 의 바닥선도 s, 바가 칠하는 깊이도 s 다. 셋이 다르면
            //   그 차이만큼 바탕화면이 비치거나 남의 창 가장자리를 우리가 삼킨다.
            double at = outer, used = 0, solid = 0;
            for (int i = 0; i < rows.Count; i++)
            {
                double t = RowThick(rows[i]);
                if (used + t > total) t = total - used;
                if (t < 4) break;
                double s = SolidThick(rows[i]);
                if (s > t) s = t;
                PlaceRow(rows[i], Dock.BandAt(free, edge, at, t), edge, s);
            ...
            if (lead != null)
            {
                ...
                {
                    slot.Holder = lead;
                    // 호버로 커질 자리까지 뺏으면 평소에 빈 띠가 남는다. 속이 찬 만큼만 청한다.
                    // ★ 이 값은 바들이 칠하는 깊이와 반드시 같은 값이다 ★
                    //   예전에는 '줄 두께 - 그 줄의 가장 큰 호버 몫' 으로 뺐다. 그 두 값은
                    //   서로 다른 바가 정하는 것이라(두께는 시세, 호버 몫은 즐겨찾기)
                    //   181 을 칠하면서 125 만 청하고 있었다.
                    double keep = solid;
                    if (keep < 8) keep = used;
        ...
        /// </summary>
        /// <summary>
        /// 이 변에서 우리가 화면에서 **실제로 뺏은** 깊이.
        ///
        /// ★ 창 두께가 아니라 확보한 두께다 ★
        ///   둘은 다르다. 창은 호버로 커질 자리까지 품고 있어 확보한 띠보다 두껍다(즐겨찾기).
        ///   창 두께로 되돌리면 그 차이만큼 남의 자리까지 내 것이라고 우기는 셈이라,
        ///   세로 바가 작업표시줄 위로 파고들어 시작 단추를 가리고 클릭까지 삼킨다.
        ///   Layout 이 ReserveSpan 에 넘기는 식(keep = solid, 너무 얇으면 used)과 같아야 한다.
        ///
        ///   한 가지 어긋날 수 있는 자리가 있다 - Layout 은 바가 변의 45% 를 넘으면 total 을
        ///   잘라낸다. 여기서는 그 상한을 모른다(화면을 안 받는다). 그때는 여기가 더 크게
        ///   나오지만, 그 지경이면 이미 화면 절반이 바라 모서리 몇 px 는 문제가 아니다.
        /// </summary>
        ...
        /// <summary>창을 앉히고, 그 자리와 '칠할 깊이' 를 적어 둔다.</summary>
        private static void Put(IDockBar b, Rect band, double solid)
        {
            _placed[b] = band;
            _solid[b] = solid;   // ★ PlaceBar 안에서 되읽으므로 먼저 적는다 ★
            try { b.PlaceBar(band); }
            catch { }
        }

        /// <summary>
        /// 시세가 있는 줄은 시세의 두께를 모든 불투명 배경의 공통 기준으로 쓴다.
        /// 즐겨찾기 아이콘의 투명한 호버 공간만 이 기준 밖으로 나올 수 있다.
        /// </summary>
        /// <summary>
        /// 그 줄의 '속이 찬' 두께. 호버로 커질 몫은 뺀 값이다.
        ///
        /// 시세가 없을 때는 조각들의 가장 두꺼운 불투명 영역을 기준으로 정한다.
        /// </summary>
        private static double SolidThick(List<IDockBar> row)
        {
            // The quotes bar is the base of a joined row. Followers keep their text scale,
            // but must share its painted edge rather than enlarge it with their own scale.
            var basis = row.Find(b => b.BarOrder == 0);
            if (basis != null) return Math.Max(4, Thick(basis) - basis.BarOverhangPx);
            double t = 0;
        ...
        {
            double own = b.BarOverhangPx > 0 ? Thick(b) : floor;
        ...
        /// <summary>이 바가 칠해야 할 깊이 (프로세스 픽셀). 아직 자리를 못 받았으면 0.</summary>
        public static double SolidPxOf(IDockBar b)
        {
            double s;
            if (b == null || !_solid.TryGetValue(b, out s)) return 0;
            return s;
        }
        ...
        // OverhangOf 는 지웠다. '안 뺏을 몫' 은 SolidThick 하나로 표현된다 -
        // 줄 두께에서 max(호버 몫)을 빼는 것과 max(두께 - 호버 몫)은 같은 값이 아니고,
        // 그 차이가 곧 '칠한 만큼 못 청하는' 오차였다.

        /// <summary>길이를 안 밝힌 바(시세)에게 최소한 남겨 두는 길이. 이보다는 줄이지 않는다.</summary>
        private const double FillerFloor = 120;

        /// <summary>공통 배경 두께에 투명한 아이콘 호버 영역만 추가한다.</summary>
        private static double RowThick(List<IDockBar> row)
        {
            double t = SolidThick(row);
            for (int i = 0; i < row.Count; i++)
            {
                if (row[i].BarOverhangPx > 0) t = Math.Max(t, Thick(row[i]));
            }
            return t < 4 ? 4 : t;
        }
        ...
        /// </summary>
        private static void PlaceRow(List<IDockBar> row, Rect band, DockEdge edge, double solid)
        {
            if (row.Count == 0) return;
            if (row.Count == 1) { Put(row[0], Fit(row[0], band, edge, solid), solid); return; }

            bool vert = (edge == DockEdge.Left || edge == DockEdge.Right);
            double span = vert ? band.Height : band.Width;
            if (span < row.Count) return;

            double floor = solid;   // 얇은 바도 여기까지는 늘려 턱을 없앤다. 확보한 깊이와 같은 값이다.
            ...
            var filler = new bool[row.Count];
            for (int i = 0; i < row.Count; i++) filler[i] = (want[i] <= 0);

            if (fillers > 0)
            {
                double left = span - sum;
                double each = left / fillers;
                if (each < FillerFloor) each = FillerFloor;
                for (int i = 0; i < row.Count; i++)
                    if (filler[i]) { want[i] = each; sum += each; }
            }
            if (sum <= 0) return;

            // ★ 모자라면 '채우는 쪽' 이 먼저 진다 ★
            //   시세는 들어가는 만큼만 담고 나머지는 생략하도록 되어 있어(AddDockItem)
            //   적게 받아도 멀쩡하다. 남는 자리를 나눌 때(위의 each) 이미 시세가 다
            //   뒤집어썼고 FillerFloor 에서 멈춘다. 여기서 한 번 더 깎던 고리는 지웠다 -
            //   each 가 이미 FillerFloor 로 눌려 있어 깎을 몫이 언제나 0 인 죽은 코드였다.
            //   그러고도 모자라면 아래 scale 이 모두를 비례로 줄인다. 그때 길이를 밝힌 바는
            //   '받은 길이' 를 알고 알맹이를 다시 담는다(PanelWindow.PlaceBar 의 RefitSoon).

            // ★ 가운데 모으기는 여기서 하지 않는다 ★
            //   BarCentered 분기를 지웠다. 가운데 바에게 span - 2*max(before,after) 를 주면
            //   그 길이는 '제가 청한 길이' 가 아니라 '이웃의 길이' 다. 이웃이 바뀔 때마다
            //   즐겨찾기가 제 알맹이보다 짧아져 아이콘이 잘렸고, before != after 인 만큼은
            //   어느 창도 안 덮어 띠 끝에 바탕화면이 남았다.
            //   가운데로 보이게 하는 것은 알맹이 쪽에서 이미 하고 있다
            //   (BuildPanelBar: box.VerticalAlignment = Center).
            ...
            for (int i = 0; i < row.Count; i++)
            {
                ...
                Put(row[i], Fit(row[i], r, edge, floor), solid);
                at += L;
            }
        }
