...
namespace DeskWidget
{
    internal sealed class PanelWindow : Window, IDockBar
    {
        ...
        private Brush BarBackdrop(bool horizontal)
        {
            ...
            // ★ 좌·우에 붙었을 때는 시세 바의 배경을 이어받는다 ★
            //   Card 는 그라데이션이라 요소마다 제 높이에 맞춰 다시 칠해진다. 짧은 바에
            //   그대로 주면 경계에서 색이 되돌아가 줄이 보인다(Palette.CardEnd 주석).
            //   세로 줄에서는 이 바들이 시세 아래에 오므로, 시세가 끝나는 색으로 칠하면
            //   이어져 보인다.
            if (!horizontal) return Palette.CardEnd;
