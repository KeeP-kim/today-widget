...
namespace DeskWidget
{
    internal sealed class PanelWindow : Window, IDockBar
    {
        ...
        public void Undock()
        {
            ...
            //   SizeToContent 라 지금은 크기가 아직 안 잡혔다.
            //
            //   ★ 되돌린 자리가 있으면 지금은 손대지 않는다 ★
            //     이 시점의 ActualWidth 는 아직 '바' 의 폭, 곧 화면 전체다. 그 크기로 재면
            //     x + w > R 이 늘 참이라 Left 가 0 으로 눌린다 - 아래 바를 뗄 때마다 카드가
            //     제자리가 아니라 화면 왼쪽 끝에 나타나던 것이 이것이다. 그리고 그 값은
            //     읽고 마는 것이 아니라 Left 에 실제로 써지므로, 뒤에 크기가 잡힌 뒤 다시
            //     불러도 이미 화면 안이라 되돌려 주지 않는다.
            //     되돌릴 자리가 없을 때만 대충이라도 들여놓는다 - 그때는 붙어 있던 가장자리
            //     좌표 그대로라 두면 카드가 화면 밖으로 걸친다.
            if (!restored) ClampIntoScreen();
            try
            {
        ...
        public bool ClearBackdrop
        {
            ...
            set
            {
                ...
                // ★ 칠하는 곳은 두 군데다 - 여기서도 마찬가지다 ★
                //   예전에는 _barBox 만 다시 칠했다. 그러면 배경은 알파 0 이 되는데
                //   잡는 띠(_barGrab)는 접힌 채로 남아, 창에 불투명한 픽셀이 글자 획과
                //   6px 손잡이밖에 없게 된다. 알파 0 은 '투명한 픽셀' 이 아니라 '창에 없는
                //   픽셀' 이라 OS 가 마우스를 아래 창으로 흘려보낸다 - 끌 수도, 우클릭해
                //   되돌릴 수도 없어진다. RefreshBackdrop 이 둘을 함께 고른다.
                RefreshBackdrop();
            }
        }
        ...
        private void PlaceBarGrip(bool vertical)
        {
            ...
            RefreshBackdrop();   // 배경과 '잡는 띠' 를 함께 고른다 - 둘은 한 결정의 두 면이다
        }
        ...
        private void RefreshBackdrop()
        {
            ...
            // ★ 칠하는 곳은 두 군데다 ★
            //   즐겨찾기는 _barBox 가 늘 Clear 라(BarBackdrop 위쪽) _barGrab 이 유일한
            //   배경이다. 예전에는 여기서 _barBox 만 다시 칠했고, _barGrab 은 RebuildBar
            //   때 한 번 고른 색에 굳어 있었다. 그 시점에는 _dockScreen 이 아직 없어
            //   HasCompanion 이 늘 거짓 -> Palette.Grab(알파 1, 눈에는 바탕화면).
            //   그래서 옆 바는 까맣고 즐겨찾기 칸만 바탕화면이 비쳤다.
            try { _barBox.Background = BarBackdrop(!vertical); }
            catch { }
            try { PlaceBarGrab(vertical); }
            catch { }
        }
        ...
        private void PlaceBarGrab(bool vertical)
        {
            ...
            // ★ 옆에 다른 바가 있으면 이 띠가 배경 노릇을 한다 ★
            //   즐겨찾기 바는 창 전체를 칠하지 않는다(호버로 커질 몫이 남의 자리 위에 떠 있다).
            //   그래서 '확보한 띠' 인 여기에만 시세가 끝나는 색을 깐다 - 그러면 옆 바와
            //   높이가 어긋나지 않으면서 한 줄로 이어진다.
            //   혼자일 때는 알파 1 만 남겨 잡히기만 하고 보이지는 않는다.
            //
            //   다만 '바 배경 없애기' 를 켰으면 옆에 누가 있든 알파 1 만 남긴다.
            //   지우라고 한 것을 이어 붙이자고 도로 칠하면 그 메뉴가 아무 일도 안 한 것처럼 보인다.
            _barGrab.Background = (!_clearBackdrop && DockStack.HasCompanion(this))
                                ? Palette.CardEnd : Palette.Grab;

            // ★ 깊이는 내가 정하지 않는다 ★
            //   창 두께는 줄에서 가장 두꺼운 바에 맞춰 늘어난다(DockStack.Fit). 제 두께로
            //   칠하면 늘어난 몫이 알파 0 으로 남아, 옆 바는 꽉 찼는데 여기만 바탕화면이
            //   비친다(실측: 181px 창에 112px 만 칠하고 있었다).
            //   자리를 잡아 준 쪽이 확보한 깊이를 그대로 받아 칠한다.
            double keep = SolidDip(vertical);
            if (keep < 4) keep = 4;
        ...
        /// 칠해야 할 깊이 (DIP).
        ///
        /// DockStack 이 이 줄에 확보한 깊이를 그대로 쓴다 - 확보한 띠와 칠한 띠가 같아야
        /// 앞뒤가 맞는다. 아직 자리를 못 받았으면 제 두께에서 커질 몫만 뺀 값으로 시작하고,
        /// 첫 PlaceBar 가 곧 고쳐 준다.
        /// </summary>
        private double SolidDip(bool vertical)
        {
            double sx, sy;
            Dock.GetDpiScale(this, out sx, out sy);
            double axis = vertical ? sx : sy;

            double px = DockStack.SolidPxOf(this);
            if (px > 0 && axis > 0) return px / axis;
        ...
        // BarCentered 는 지웠다. 가운데로 모으는 것은 알맹이가 이미 하고 있고
        // (BuildPanelBar 의 box.VerticalAlignment = Center), 자리 나누기에서까지 하면
        // 이웃의 길이로 제 길이가 정해져 아이콘이 잘렸다.
        ...
        int IDockBar.BarLengthPx
        {
            get
            {
                try
                {
                    ...
                    // 잘라 담았으면 잰 값이 실제보다 짧다. 온전한 길이로 청한다.
                    if (!double.IsNaN(_naturalDip) && _naturalDip > dip) dip = _naturalDip;
                    if (dip < 1) return 0;
        ...
        void IDockBar.PlaceBar(Rect procPx)
        {
            ...
            try
            {
                ...
                if (h != IntPtr.Zero)
                {
                    ...
                    RefreshBackdrop();   // 이웃과 깊이가 정해진 뒤라야 배경·띠를 제대로 고른다

                    // ★ 받은 길이가 바뀌면 알맹이를 다시 담는다 ★
                    //   시세 바에는 이미 있던 규칙이다(WidgetWindow.PlaceBar). 조각 바에는
                    //   없어서, 짧게 받은 즐겨찾기가 '줄기 전 길이' 로 만든 아이콘 줄을
                    //   그대로 갖고 있다가 창 밖으로 넘쳐 잘렸다. 자리를 잡는 중이므로 미룬다.
        ...
        private double _naturalDip = double.NaN;

        /// <summary>받은 길이가 바뀌었으니 알맹이를 다시 담는다. 자리는 다시 안 나눈다.</summary>
        private void RefitSoon()
        {
            ...
            try
            {
                ...
                {
                    _refitPending = false;
                    if (Edge != DockEdge.None) RebuildBar();
                });
            }
            catch { _refitPending = false; }
        }

        /// <summary>이 바가 실제로 받은 길이 (DIP). 아직 못 받았으면 NaN.</summary>
        public double BarFitDip(bool vertical)
        {
            if (_placedLen <= 0) return double.NaN;
            double sx, sy;
            Dock.GetDpiScale(this, out sx, out sy);
            double axis = vertical ? sy : sx;
            if (axis <= 0) return double.NaN;
            return _placedLen / axis;
        }

        /// <summary>
        /// 알맹이가 '자르기 전에' 바라던 길이 (DIP).
        ///
        /// ★ 잘라 담은 길이로 다시 청하면 안 된다 ★
        ///   짧게 받아 -> 몇 개 빼고 -> 그만큼만 청하면 다음 배치에서 더 짧게 받고 또 뺀다.
        ///   아이콘이 한 번에 하나씩 사라진다. 청하는 값은 늘 온전한 길이여야 한다.
        /// </summary>
        public void SetBarNaturalDip(double dip)
        {
            _naturalDip = dip;
        }
        ...
        // 저장·복원 쪽과 같은 상수를 쓴다 - 갈라지면 줄인 것이 다음 실행에 사라진다.
        public const double MinScale = Config.MinPanelScale;
        public const double MaxScale = Config.MaxScale;
