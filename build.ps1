...
if ($AnalysisOnly) {
    $analysisExe = Join-Path $root 'Onuln.Analysis.exe'
    & $csc (@('/nologo', '/target:winexe', '/main:DeskWidget.DollarProgram', '/platform:x64',
              '/optimize+', '/codepage:65001', "/out:$analysisExe", "/win32icon:$icon") + $refs + $sources)
    if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
    Write-Host '[완료] Onuln.Analysis.exe'
    exit 0
}
...
# ★ 파일이 '있는지' 로 성공을 판정하면 안 된다 ★
#   컴파일이 깨져도 지난번 산출물이 그대로 남아 있어 Test-Path 는 통과한다.
#   그러면 [완료] 를 찍어 놓고 옛 코드가 계속 돌아, 고친 것이 안 먹는 것처럼 보인다.
#   실제로 여기서 한 번 속았다. 컴파일러의 종료 코드를 본다.
if ($LASTEXITCODE -ne 0) { Write-Host "[실패] DLL 컴파일 오류" -ForegroundColor Red; exit 1 }
if (-not (Test-Path $dll)) { Write-Host "[실패] DLL 이 만들어지지 않았다" -ForegroundColor Red; exit 1 }

# 2) EXE - Smart App Control 이 꺼진 PC 에서 직접 실행할 때 쓴다.
& $csc (@('/nologo', '/target:winexe', '/main:DeskWidget.Program', '/platform:x64', '/optimize+', '/codepage:65001',
          "/out:$exe", "/win32icon:$icon") + $refs + $sources)
if ($LASTEXITCODE -ne 0) { Write-Host "[실패] EXE 컴파일 오류" -ForegroundColor Red; exit 1 }
